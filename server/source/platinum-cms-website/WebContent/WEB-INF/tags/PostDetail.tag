...
<div id="<%= id%>" class="PostDetail <%= post.getPostType()%>">
    <% if (post.getPostType() == PostType.NEWS || post.getPostType() == PostType.NOTIFICATION) {%>
        ...
        <div id="content">
        	<%= post.getContentText()%>
        	<div id="printLi" style="height:20px;margin-top:5px;"><input id="printButton" style="border:0;cursor:pointer;float:right;font-size:13px;font-faminly:'微软雅黑';"type="button" value="【打印】" onclick="PrintView();" /></div>
        </div>
        <% } %>
...
<script>
	...
	if($('#side').length>0) {
	 ...
	 if ($("#categoryNavigationBar li a:last").text() == "通知公告" || $("#categoryNavigationBar li a:last").text() == "通知与公告") {
